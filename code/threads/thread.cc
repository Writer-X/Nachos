...
void Thread::Join(int SpaceId) {
    ...
    waitingProcessSpaceId = SpaceId;     // 设置当前线程所等待进程的spaceId
    ...
    while(first != NULL){
        ...
        if(thread->UserProgramId == SpaceId){       // 在队列中
            ...
            waitProcessExitCode = thread->exitStatus;  // 设置父线程等待子线程退出码
            break;
        }
        first = first->next;
    }
    ...
    if(!interminatedList){
        ...
        waitingList->Append((void *)this);  // Joiner加入等待队列
        currentThread->Sleep();             // joiner睡眠
    }
    //Joinee在终止队列中，在终止队列中删除Joinee
    else
    {
...
//----------------------------------------
//Terminated:将一个进程终止并加入终止队列的具体代码。
//------------------------------------------
void
Thread::Terminated()
{
...
//----------------------------------------
void 
Thread::setExitStatus(int ExitStatus)
{
...
Thread::Finish ()
{
    ...
    currentThread->UserProgramId = currentThread->space->getSpaceID();
    while(first != NULL){
        Thread *waitingThread = (Thread *)first->item;    
        printf("%d %d\n",currentThread->UserProgramId,waitingThread->waitingProcessSpaceId);
