...
{ 
    readyList = new List;
#ifdef USER_PROGRAM
    waitingList = new List;    //线程等待队列,如果Joinee没有退出，Joiner进入等待
    terminatedList = new List; //线程终止队列,线程代用Finish（）后进入该状态
                               //Joiner通过检查该队列确定Joinee是否已经退出
...
//-------------------------------------------
//deleteTreminatedThread:从Scheduler 的终止队列中删去一个线程
//-------------------------------------------
void 
