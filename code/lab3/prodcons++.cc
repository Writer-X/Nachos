...
Thread *producers[N_PROD]; //array of pointers to the producer  //生产者线程数组
...
Semaphore *nempty, *nfull; //two semaphores for empty and full slots         //初始化信号量和环形缓冲区
...
//   page 182 of the textbook.
    ...
    for (num = 0; num < N_MESSG ; num++) {
      ...
      // ...
      message->thread_id = which;   //message赋值
      message->value = num;
...
ProdCons()
{
    ...
    // ....
    nempty = new Semaphore("nempty", BUFF_SIZE);    //信号量
